...
class Player extends \BANG\Helpers\DB_Manager
{
 ...
 protected static $primary = 'player_id';
  ...
  public function isAvailable($expansions){ return in_array($this->expansion, $expansions); }
...
//          Cards::discardCard($card);
  ...
  public function attack($playerIds, $checkMissed = true) {
    ...
    if(count($reactions) > 0) {
      ...
      $inactive = count($reactions) > 1 ? clienttranslate('Players may react to ${src_name}') : clienttranslate('${actplayer} may react to ${src_name}');
      $args = [
